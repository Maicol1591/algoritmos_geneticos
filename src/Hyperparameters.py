...
class GeneticAlgorithmOptimizer:
    def __init__(self, cantidad_generaciones, cantidad_ciudades):
        self.cantidad_generaciones = cantidad_generaciones
        self.cantidad_ciudades = cantidad_ciudades
    ...
    def objective(self, trial):
        ...
        cantidad_poblacion = trial.suggest_int('cantidad_poblacion', 40, 80)
        elite_size = trial.suggest_int('elite_size', 5, 30)
        mutation_rate = trial.suggest_float('mutation_rate', 0.01, 0.31, step=0.05)

        # Initial population generation
        poblacion_inicial = gen.crear_poblacion_inicial_random(cantidad_poblacion, self.cantidad_ciudades)

        # Run the genetic algorithm with the current hyperparameters
        _, mejor_distancia, _, _ = gen.genetic_algorithm(
            cantidad_poblacion,
            self.cantidad_ciudades,
            poblacion_inicial,
            mutation_rate,
            self.cantidad_generaciones, 
            elite_size,
            metodo_seleccion="Ruleta",
            cantidad_participantes_torneo=10
        )

        # Return the objective value to be minimized (or maximized)
